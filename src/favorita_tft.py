...
from .configs import DEFAULT_TFT_DATASET_CONFIG, DEFAULT_TFT_PARAMS, DEFAULT_TRAINER_PARAMS
...
from .favorita_eda_utils import CACHE_DIR, DATA_DIR, SUBMISSIONS_DIR, _cache_path, build_train_eda_bundle
...
def _configure_tft_warning_filters() -> None:
    """Suppress known non-critical warnings to keep training logs readable."""
    ...
    warnings.filterwarnings(
        "ignore",
        message=r"Min encoder length and/or min_prediction_idx.*not present in the dataset index",
        category=UserWarning,
    )
    warnings.filterwarnings(
        "ignore",
        message=r"Found \d+ unknown classes which were set to NaN",
        category=UserWarning,
    )
    warnings.filterwarnings(
        "ignore",
...
def _build_tft_datasets(
    ...
    _configure_tft_warning_filters()
    ...
    min_encoder_length = int(
        dataset_config.get(
            "min_encoder_length",
            min(max_encoder_length, max(int(dataset_config.get("min_history_points", 1)), 1)),
        )
    )
    min_encoder_length = max(1, min(min_encoder_length, max_encoder_length))
...
def _fit_tft_model(
    ...
    trainer = pl.Trainer(
        ...
        # Some CUDA ops used by TFT have no strict deterministic backward path.
        # "warn" avoids RuntimeError while preserving reproducibility warnings.
        deterministic="warn",
...
def run_single_fold_tft_experiment(
    ...
    dataset_cfg.update(
        {
            "max_encoder_length": max_encoder_length,
            "min_encoder_length": max(1, min(min_history_points, max_encoder_length)),
            "max_prediction_length": max_prediction_length,
            "max_series": max_series,
            "min_history_points": min_history_points,
...
def train_final_tft_model(
    ...
    submission_path = SUBMISSIONS_DIR / f"submission_tft_lb{lookback_days}_enc{max_encoder_length}.csv.gz"
