...
from .. import favorita_tft as ft
...
def load_state_dict_into_model(model: Any, state_dict_path: Path) -> dict[str, Any]:
    ...
    ft._require_tft_dependencies()
    used_weights_only_fallback = False
    try:
        # torch>=2.6 defaults to weights_only=True and may reject full Lightning checkpoints.
        payload = ft.torch.load(state_dict_path, map_location="cpu")
    except Exception as exc:
        if "Weights only load failed" not in str(exc):
            raise
        payload = ft.torch.load(state_dict_path, map_location="cpu", weights_only=False)
        used_weights_only_fallback = True
    ...
    return {
        ...
        "used_weights_only_fallback": used_weights_only_fallback,
    }
...
def fit_tft_model(
    ...
    ft._require_tft_dependencies()
    if hasattr(ft, "_configure_tft_warning_filters"):
        ft._configure_tft_warning_filters()
    ...
    trainer = ft.pl.Trainer(
        ...
        # Some CUDA ops in TFT do not support strict deterministic backward.
        # "warn" avoids hard crashes while still reporting determinism issues.
        deterministic="warn",
